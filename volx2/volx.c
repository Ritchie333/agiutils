...
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h>		// not ANSI-C, but mkdir() is very common
//#include "agi.h"
...
unsigned int lhconv( const unsigned char l, const unsigned char h )
{
	return ( h * 0x100 ) + l;
}
...
void extractAll(int fileNameNum)
{
	...
	mkdir( fileNames[fileNameNum], S_IRWXU );
