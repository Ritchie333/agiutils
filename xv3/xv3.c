...
int loadGameSig(char *gameSig)
{
   ...
   if( dp ) {
      while (( ep = readdir( dp )) ) {
         if( ( end = strstr(ep->d_name, "DIR") ) != NULL)
            strncpy(dirString,ep->d_name, end - ep->d_name);
         if( ( end = strstr(ep->d_name, "VOL.0")) != NULL)
            strncpy(volString,ep->d_name, end - ep->d_name);
      }
      closedir( dp );
   }
   ...
   if ((strcmp(volString, dirString) == 0) && strlen(volString) > 0 && strlen(dirString) > 0 ) {
...
void extractObjFile()
{
	...
	if( ! fread( buffer, objlength, 1, objfile ) ) {
		printf( "Read error!\n" );
		fclose( objfile );
		fclose( outfile );
		free( buffer );
	...
	if( !fwrite( buffer, objlength, 1, outfile ) ) {
		printf( "Error!\nWrite error!\n" );
		fclose( objfile );
		fclose( outfile );
		free( buffer );
