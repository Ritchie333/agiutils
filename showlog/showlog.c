...
boolean DEBUG = FALSE;
byte numMessages = 0;
char *messages[MAX_BUF];

word logPos = 0;
char *logData[MAX_BUF];

int numObjects = 0;
char *objects[MAX_BUF];
...
void addToLine(const char *stringToAdd);
...
int calcNumWords()
{
   ...
   while (!feof(wordFile)) {
      data = fgetc(wordFile);
      if (data > 0x80) {
	     wordNum = data + 256 * (byte)fgetc(wordFile);
	    if (wordNum > max)
	 	   max = wordNum;
      }
   }
...
void loadObjectNames()
{
   ...
   marker = (objData = (byte *)malloc(fileLen * sizeof(byte))) + 3;
...
char debugLine[1024], debugChar[15];
...
void displayMessages()
{
   ...
   char tempString[1024], messString[256];
